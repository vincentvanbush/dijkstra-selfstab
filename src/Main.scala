...
object Main extends App {
  ...
  val system = ActorSystem("dijkstra", ConfigFactory.load(config))
  ...
  def runRemote = {
    ...
    val nodes = ports.map(p => {
      Await.result(system.actorSelection(s"akka.tcp://dijkstra@$ip:$p/user/Node").resolveOne(10 seconds), 10 seconds)
    })
